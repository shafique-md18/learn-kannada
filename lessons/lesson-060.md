# Lesson 060: Small Talk Combo

## Quick Recall

Say in Kannada:

1. Coffee ge barthira?
2. Coffee ge hogona.
3. Sanje free ideya?

## New Words

### Must Memorize

| Kannada | Hindi Pronunciation | Hindi Meaning | English Meaning | Bangalore Usage |
|---|---|---|---|---|
| hege | हेगे | कैसा | how | Bangalore hege ide? |
| weather | वेदर | मौसम | weather | Weather chennagide. |
| area | एरिया | इलाका | area | Area chennagide. |
| safe | सेफ | सुरक्षित | safe | Area safe ide. |
| expensive | एक्सपेंसिव | महंगा | expensive | Bangalore expensive. |

## Main Pattern

### Kannada Sentence

Bangalore hege ide?

### Hindi Pronunciation

बैंगलोर हेगे इदे?

### Meaning

Hindi: बंगलौर कैसा है?

English: How is Bangalore?

## Word-by-Word Breakdown

| Word | Hindi Meaning | English Meaning |
|---|---|---|
| Bangalore | बंगलौर | Bangalore |
| hege | कैसा | how |
| ide | है | is there |

## Sentence Construction Logic

This lesson extends the reusable pattern `___ hege ide?`. Keep the sentence short first, then add time/place/person words.

## Reusable Pattern

`___ hege ide?`

Examples:

- Bangalore hege ide?
- Weather chennagide.
- Area safe ide.
- Bangalore expensive.

## Guided Practice

Build these in Kannada:

1. How is Bangalore?
2. Weather is good.
3. Area is safe.
4. Bangalore is expensive.

## Production Practice

Say these in Kannada:

1. In a real Bangalore situation: How is Bangalore?
2. Change the place/person/item and say: Weather is good.
3. Use as the next line in a short conversation: Area is safe.
4. Combine old knowledge with today: Bangalore is expensive.

## Real-Life Bangalore Roleplay

Have small talk with a neighbour about Bangalore and weather.

## Quiz

1. Say: How is Bangalore?
2. Say: Weather is good.
3. Say: Area is safe.
4. Say: Bangalore is expensive.

## Add These To Your Notes

Add:

- ___ hege ide? = core pattern
- Bangalore hege ide?
- Weather chennagide.
